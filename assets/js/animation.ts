...
const stages: Stage[] = [
  {
    name: "Type-Safe RPC",
    description:
      "Auto-generated TypeScript functions for every Ash action with full type inference.",
...
if (result.success) {
  ...
  },
  {
    ...
    description:
      "Typed route helpers and fetch functions for Phoenix controllers.",
...
const result = await login({
  ...
  },
  {
    name: "Typed Channels",
    description:
      "Type-safe Phoenix channel event subscriptions with Ash PubSub.",
...
const refs = onOrgChannelMessages(channel, {
  ...
  },
  {
    ...
    description:
      "Request exactly the fields you need with full type narrowing.",
...
function highlightElixir(code: string): string {
  ...
  const patterns: [RegExp, string][] = [
    ...
    [/"[^"]*"/g, "text-yellow-400"],
    [
      /\b(defmodule|def|defp|do|end|use|fn|if|else|case|cond|with|for|unless|import|alias|require)\b/g,
      "text-pink-400",
    ],
...
function highlightTS(code: string): string {
  ...
  const patterns: [RegExp, string][] = [
    ...
    [/`[^`]*`/g, "text-yellow-400"],
    [
      /\b(import|from|export|const|let|var|function|return|if|else|async|await|new|typeof|interface|type)\b/g,
      "text-pink-400",
    ],
...
function typewrite(
  ...
  let idx = 0
  el.innerHTML =
    '<span class="inline-block w-[2px] h-[1.1em] bg-primary align-text-bottom animate-pulse"></span>'
...
export function initLandingPage(container: HTMLElement): () => void {
  ...
  const docsLink =
    container.querySelector<HTMLAnchorElement>("#stage-docs-link")!
...
function buildHTML(): string {
  const dotHTML = stages
    .map(
      (s, i) =>
        `<button class="stage-dot w-3 h-3 rounded-full transition-all duration-300 cursor-pointer ${i === 0 ? "bg-primary opacity-100 scale-125" : "bg-foreground opacity-30"}" title="${s.name}"></button>`,
    )
    .join("")
